...
<h1 class="container text-center mb-3 header-text"
        ...
        <thead>
                <tr>
                    <th class="text-light bg-dark">No. Pembelian</th>
                    <th class="text-light bg-dark">ID Buku</th>
                    <th class="text-light bg-dark">NISN</th>
                    <th class="text-light bg-dark">Tanggal Peminjaman</th>
                    <th class="text-light bg-dark">Tanggal Pengembalian</th>
                    <th class="text-light bg-dark">Status</th>
                    <th class="text-light bg-dark">Aksi</th>
                </tr>
            </thead>
  ...
  <?php endif; ?></td>
    ...
    <?php if ($data['bukti_transaksi']) : ?>
      <div class="butt" style="margin-right:10px ;">
  <a href="konfirmasi_peminjaman.php?id=<?= $data['id_peminjaman']; ?>" class="btn btn-dark" onclick="return confirm('Yakin Bukti Sudah Benar ?');">Konfirmasi </a> 
  </div>
  <?php else : ?>
    <button disabled href="konfirmasi_peminjaman.php?id=<?= $data['id_peminjaman']; ?>" class="btn btn-dark text-light">Konfirmasi </button> 
    <?php endif; ?>
    <form action="" method="post">
    <input type="hidden" name="id_peminjaman" value="<?= $data["id_peminjaman"]; ?>">
    <button type="submit" name="delete" class="btn btn-dark" onclick="return confirm('Yakin ingin menghapus data peminjaman?');">Tidak Sesuai</button>
  </form>
  </div>
</td>
